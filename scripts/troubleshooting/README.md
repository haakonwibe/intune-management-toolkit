...
> Today the script is a *reporter* (it always exits 0 / writes to host). To gate a remediation on health, see **Future enhancements → automation-friendly exit codes** below.
...
- **Automation-friendly exit codes / output.** Optional `-AsRemediation` mode that emits a single-line status and a non-zero exit when `FAIL > 0`, so it works cleanly as a Proactive Remediation detection script.
